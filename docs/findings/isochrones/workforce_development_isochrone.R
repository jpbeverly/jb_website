...
# work_plot <- leaflet(data = workforce_dev_center_data[c(4,8),]) %>% # create leaflet object
...
map_for_points <- mapview(sf_work_dev_center, cex =2,color="orange", col.regions= "orange", layer.name= "Workforce Development Centres")
map_for_points
...
# wrk_dev_iso_20_index_4 <- traveltime_map(appId= traveltime_id,
#                                          apiKey = traveltime_api,
#                                          location=c(workforce_dev_center_data$Latitude[4], workforce_dev_center_data$Longitude[4]),
#                                          traveltime=1200,
#                                          type="driving",
...
# saveRDS(wrk_dev_iso_20_index_4, file = paste0('wrk_dev_iso_20_index_4','.RDS'))
# # this particular isochrone is for a 10 minute travel window
#
# wrk_dev_iso_20_index_8 <- traveltime_map(appId= traveltime_id,
#                                          apiKey = traveltime_api,
#                                          location=c(workforce_dev_center_data$Latitude[8], workforce_dev_center_data$Longitude[8]),
#                                          traveltime=1200,
#                                          type="driving",
#                                          departure="2020-08-07T08:00:00+01:00")
# # we save each object as an RDS file because the api limits the amount of calls you can do in a given period of time and because we don't want to waste our time looping through this whenever we want to work.
# saveRDS(wrk_dev_iso_20_index_8, file = paste0('wrk_dev_iso_20_index_8','.RDS'))
# # this particular isochrone is for a 10 minute travel window
...
wrk_dev_iso_20_index_4 <- readRDS("wrk_dev_iso_20_index_4.RDS")
wrk_dev_iso_20_index_8 <- readRDS("wrk_dev_iso_20_index_8.RDS")
